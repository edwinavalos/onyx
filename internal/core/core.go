...
// VolumeInfo is one volume and the definitions that attach it.
...
func (c *Core) ListVolumes() ([]VolumeInfo, error) {
	...
	for _, n := range names {
		info := VolumeInfo{Name: n, VMs: []string{}}
		if st, err := os.Stat(c.root.VolumePath(n)); err == nil {
			info.SizeMB = st.Size() / (1024 * 1024)
		}
		if u := users[n]; u != nil {
			info.VMs = u
		}
