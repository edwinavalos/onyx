...
func New(cl *client.Client, root store.Root) *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "snapshot_vm", Description: "Save a running VM's memory and device state on the host and stop it; start_vm restores it exactly. Unlike suspend_vm this needs nothing from the guest, but the VM definition must not change before restore."}, t.vmAction("snapshot"))
